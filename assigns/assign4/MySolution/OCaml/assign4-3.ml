...
let rec gtree_streamize_dfs_helper (xs: 'a gtree list): 'a stream =
  match xs with
  | [] -> fun () -> StrNil
  | GTnil :: rest -> gtree_streamize_dfs_helper rest
  | GTcons (x, children) :: rest ->
    let tail_stream = gtree_streamize_dfs_helper children in
    fun () -> StrCons (x, fun () -> gtree_streamize_dfs_helper rest () |> StrCons (x, tail_stream))

(* Depth-first traversal of gtree *)
let rec gtree_streamize_dfs (xs: 'a gtree): 'a stream =
  match xs with
  | GTnil -> fun () -> StrNil
  | GTcons (x, children) ->
    let tail_stream = gtree_streamize_dfs_helper children in
    fun () -> StrCons (x, tail_stream)

(* Helper function for breadth-first traversal *)
let rec gtree_streamize_bfs_helper (queue: 'a gtree list): 'a stream =
  match queue with
  | [] -> fun () -> StrNil
  | GTnil :: rest -> gtree_streamize_bfs_helper rest ()
  | GTcons (x, children) :: rest ->
    let child_streams = List.map (fun c -> gtree_streamize_bfs_helper [c]) children in
    let child_stream = List.fold_left (fun acc s -> fun () -> StrCons (s (), acc)) (fun () -> StrNil) child_streams in
    fun () -> StrCons (x, fun () -> gtree_streamize_bfs_helper (rest @ children) ())

(* Breadth-first traversal of gtree *)
