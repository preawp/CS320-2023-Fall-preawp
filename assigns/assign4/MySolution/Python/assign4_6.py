...
class StreamGenerator:
    ...
    def yield_next(self):
        if self.current:
            value = self.current.cons1
            self.current = self.current.cons2()
            return value
        return None

def cubesum_helper(l):
    i = l[0]
    j = l[1]
    return (i*i*i) + (j*j*j)

def cubesum_iter(l1, l2):
    return (cubesum_helper(l1) <= cubesum_helper(l2))


def strcon_merge(stream1, stream2, lte_func):
    val1 = stream1.cons1
    val2 = stream2.cons1
    if lte_func(val1, val2):
        return strcon_cons(val1, lambda: strcon_merge(stream1.cons2(), stream2, lte_func))
    else:
        return strcon_cons(val2, lambda: strcon_merge(stream1, stream2.cons2(), lte_func))

def stream_merge(stream_fn1, stream_fn2, lte_func):
    return lambda: strcon_merge(stream_fn1(), stream_fn2(), lte_func)


def theNatPairs_cubesum():
    def helper(i0):
        return strcon_cons((i0, i0), stream_merge(stream_tabulate(-1, lambda j0: (i0, i0+1+j0)), lambda: helper(i0+1), cubesum_iter))
    
    stream_gen = StreamGenerator()
    stream_gen.set_stream(helper(0))
    
    def generator_of_stream():
        while True:
            yield stream_gen.yield_next()
    
    return generator_of_stream()
