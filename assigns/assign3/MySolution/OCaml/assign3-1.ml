...
let rec matrix_transpose (xss: 'a list list): 'a list list =
  ...
  let cols = match xss with
    | [] -> 0
    | hd :: _ -> List.length hd
  in
  if cols = 0 then []
  else if rows = 0 then List.init cols (fun _ -> [])
  else
    let column = foreach_to_listize (int1_foreach cols) (fun j ->
      foreach_to_listize (int1_foreach rows) (fun i ->
        List.nth (List.nth xss i) j
      )
    ) in
    column :: matrix_transpose (foreach_to_rlistize (int1_foreach rows) (fun i ->
      foreach_to_listize (int1_foreach cols) (fun j ->
        List.nth (List.nth xss i) j
      )
    ))
;;
...
let () =
  ...
  let transposed = matrix_transpose matrix in
  List.iter (fun row ->
    let row_str = String.concat " " (List.map string_of_int row) in
    Printf.printf "%s\n" row_str
  ) transposed
